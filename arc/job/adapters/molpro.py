...
class MolproAdapter(JobAdapter):
    ...
    def __init__(self,
        ...
        if self.scan is not None:
            if not isinstance(self.scan[0], list):
                self.scan = [self.scan]
            if not isinstance(self.scan[0][0], list):
                self.scan = [self.scan]
        ...
        self.max_task_num, self.threads_per_task = None, None
    ...
    def write_input_file(self) -> None:
        ...
        for arg_type, arg_dict in self.args.items():
            if arg_dict:
                ...
                elif arg_type == 'keyword':
                    for key, keyword in arg_dict.items():
                        if key == 'scan_trsh':
                            input_dict['scan_trsh'] += f'{keyword} '
                        else:
    ...
    def execute(self) -> Optional[dict]:
        ...
        Returns: Optional[dict]
            May return a value only if running incore.
        """
