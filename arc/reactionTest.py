...
3    H u0 p0 c0 {1,S}""", xyz=ch2_xyz)
        ...
        p_1 = ARCSpecies(label='C6H6_b', adjlist="""multiplicity 1
...
12 H u0 p0 c0 {5,S}""", xyz=c6h6_b_xyz)
    ...
    def test_get_reactants_xyz(self):
        ...
        rxn_1.r_species = [r_1, r_2]
        rxn_1.p_species = [p_1, p_2]
...
H       1.25408721   -0.86065907   -0.09003883"""
        ...
        rxn_2 = ARCReaction(reactants=['C2H5O3'], products=['HO2', 'C2H4O'])
        rxn_2.r_species = [r_1]
        rxn_2.p_species = [p_1, p_2]
        reactants_xyz_str = rxn_2.get_reactants_xyz()
        reactants_xyz_dict = rxn_2.get_reactants_xyz(return_format='dict')
        expected_reactants_xyz_str = """C      -1.34767275   -0.49923624   -0.33663726
C      -0.11626816    0.31109153    0.01886099
O       0.75311756    0.33668222   -1.10503872
O       0.52287368   -0.30498819    1.13660168
O       1.82706586    0.34102014    1.26841629
H      -2.03918170   -0.55355098    0.51000315
H      -1.86502588   -0.06806929   -1.19940469
H      -1.07119601   -1.52646294   -0.60021751
H      -0.40133539    1.33579005    0.28224155
H       1.39425696    1.03559450   -0.88907219"""
        expected_reactants_xyz_dict = c2h5o3_xyz
        self.assertEqual(reactants_xyz_str, expected_reactants_xyz_str)
        self.assertEqual(reactants_xyz_dict, expected_reactants_xyz_dict)
...
def check_atom_map(rxn: ARCReaction) -> bool:
    ...
    for i, map_i in enumerate(rxn.atom_map):
        ...
        # did not brake, the mapping makes sense
        return True
    return False
